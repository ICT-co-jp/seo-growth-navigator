...
### ② SEO Skill + スラッシュコマンド — `.claude/`

`seo-growth-navigator` Skillが、次の 6 工程パイプラインを統括します。各コマンドは自分の工程だけを完遂し、
...
Claude Code に次のように依頼すると、Skillが自動で発火し Step 1 から走ります。
...
│   ├── skills/seo-growth-navigator/  Skill本体 (SKILL.md / USAGE.md / references/)
│   └── commands/seo/               /seo:* スラッシュコマンド 6 種
├── .agents/                        他エージェント環境向けのSkill配置 (.claude のミラー)
├── LICENSE                         MIT License
└── NOTICE                          著作権・クレジット
```
...
プロンプトインジェクション**——に対処しています。

Skillの「最上位ルール」（他のすべての指示に優先）:
...
| Skillの人間向け Quick Start | [`USAGE.md`](./.claude/skills/seo-growth-navigator/USAGE.md) |
| Skillの設計思想・全体像 | [`SKILL.md`](./.claude/skills/seo-growth-navigator/SKILL.md) |
...
- **Pull Request について** — 現状、投稿いただいたコードを精査するリソースが確保できないため、**PR の取り込みは必ずしもお約束できません**。修正や機能追加のご提案は、まず Issue でご相談いただけると助かります。
