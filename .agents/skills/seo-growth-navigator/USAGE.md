...
- Claude Code 上で `mcp__ictgrowthhacker-analytics__health_check` を呼び、`{"ga4_ok": true, "gsc_ok": true}` が返ること
...
`--engine playwright` を使う場合は Chrome ブラウザバイナリも必要:

```cmd
mcp_server\.venv\Scripts\python -m playwright install chrome
```
...
[5] H2執筆          /seo:write-section  [<h2-id>]
        ↓ 引数なし: pending な全H2を「H2-01直列→残り並列」で一括執筆
...
[ユーザー]
/seo:write-section
(引数なしで全H2を一括並列執筆。差し戻したい場合のみ /seo:write-section h2-03 のように単体実行)

[Claude]
H2-01 を親で執筆 → H2-02 以降をサブエージェント並列起動 → 全 H2 完了。
失敗があれば該当 h2-id を表示し、/seo:write-section <id> での再実行を案内。
...
| `fetch_serp.py` が exit code 2 で停止 | Google が SERP をブロック (429/503) | `--engine playwright` に切替(実 Chrome + ホーム→検索でクッキー獲得し検知回避)、それでもダメなら `--headed` を追加、または UA 変更・時間を置く |
| `fetch_serp.py --engine playwright` が `/sorry/ ` で停止 | bot 検知でブロック(IP 評価/レート) | `--headed` を付けて可視モードで再試行、または時間を置く |
