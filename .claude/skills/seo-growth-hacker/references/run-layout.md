...
| `outline-ready` | `04-outline.md` 出力済 | `/seo:write-section`(引数なしで全H2並列) または `/seo:write-section h2-01`(単体) |
| `drafting` | 最初のH2が `written` になった時点 | `/seo:write-section`(残りpendingを並列) または `/seo:write-section <次>`(単体) |
...
| `/seo:write-section` | (現状維持/`drafting`) | 単体: `drafts[<h2-id>]` を1件更新 / 並列: 完了したH2の `drafts[*]` を親が一括更新(サブエージェントは触らない) |
